/**
 * Chatbot Drinky: janela de conversa, chamada à IA e o card de receita
 * sugerida.
 *
 * O parcial é incluído pelo layout em toda página, então estas linhas iam
 * junto de cada resposta HTML do site. Como arquivo estático, viram um
 * download só, cacheado entre páginas.
 *
...
(function () {
    ...
    const config = window.DrinkeritoChatbot || {};
    const isAuthenticated = config.isAuthenticated;
    const csrfToken = config.csrfToken;
    const messageUrl = config.messageUrl;
    const saveDrinkUrl = config.saveDrinkUrl;
            ...
            fetch(messageUrl, {
                ...
                .then(async (res) => {
                    ...
                    if (data.drink_suggestion) {
                        appendDrinkCard(data.drink_suggestion, data.drink_exists, data.drink_exists_reason);
                    }
